...
const ProcurementPiechart = () => {
  ...
  const { data: procOverview, isPending } = useGetProcOverview();
  const totalJobOrder = procOverview?.total_job_type || 0;
  const totalLocalPurchase = procOverview?.total_local_purchase_order || 0;
  ...
  const data = useMemo(
    ...
    [totalJobOrder, totalLocalPurchase]
  );
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        {isPending ? (
          <div>
            <div className="relative">
              ...
              <Skeleton className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
                <Skeleton className="text-3xl font-bold text-gray-800">
                  {totalProcurements}
                </Skeleton>
      ...
      {isPending ? (
        <div className="space-y-3">
          <div className="space-y-3">
            <Skeleton className="flex items-center justify-between p-3 h-16 rounded-lg"></Skeleton>
            <Skeleton className="flex items-center justify-between p-3 h-16 rounded-lg"></Skeleton>
          </div>
        </div>
      ) : (
